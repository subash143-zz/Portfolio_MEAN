...
var points = 0;
var failedAttempt = 0;
var randomColors = ["#FF0000", "#FF7F00", "#FFFF00", "#00FF00", "#0000FF", "#2E2B5F", "#8B00FF"]
//End Section
...
document.onkeydown = function(event){
    ...
    if(clickedCoordinates.length == 5){
        ...
        if(selectedComponent && event.key == selectedComponent.type){
            ...
            audio.play();
            switchComponent(components[getRandomInt(4)], selectedComponent);
            points ++;
            updateScore();
        }
        else{
            audio = new Audio("./audio/mis_fire.mp3");
            audio.play();
            failedAttempt++;
            updateScore();
        }
    ...
    if(clickedCoordinates.length == 5){
        ...
        for(i = 0; i< components.length; i++){
            if(components[i] != selectedComponent){
                components[i].color = randomColors[getRandomInt(7)];
            }
            components[i].update();
        }
    }
...
function showGameInstructions(){
    ...
    context.fillText("Instruction: Try to press the keys on the highlightes shapes.",canvas.width/2, canvas.height/2 + 40);
    gameInstructionShown = true;
}
...
function switchComponent(component1, component2){
    var tempX = component1.x;
    var tempY = component1.y;
